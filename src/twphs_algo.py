...
__METHOD_TYPE      = ['PHS', 'OPT', 'TW', 'NONE','MYTW','MYPHS','MYPHS0','MYOPT','MYOPT0'];
...
def opt_get_dimfb(K, fb_cl, fb_oth):
    ...
    # lnVphs->root of volume in original phs; new_c=max(1.,n*(1+ln(n))/S)
    # un_rk*ln(new_c)+lnVL-> should be the value of lattice in opt-phs
    # if chosed prime ideals generate the class group(restrictions on the number), then lnVL+un_rk*ln(new_c) is the volume of the new lattice
    # (k*lnVphs - un_rk*ln(new_c)) < cst_target ->->->   lnVphs < 1/(un_rk+k)*(lnVL+un_rk*ln(new_c))
    # k is bigger,c_new is small, which means the right hand is small, k should not be too bigger, which will influence the dimesnsion of the lattice
    # the meaning of the condition is to ensure that new k is smaller than the original
    fb_all = fb_cl + fb_oth;
...
def my_tw_get_dimfb(K, fb_cl, fb_oth):
    n      = K.degree();
    r1, r2 = K.signature();
    un_rk  = get_rank_units(K);
    hR     = K.class_number()*K.regulator();
    ...
    fb_all = fb_cl + fb_oth;
    k      = len(fb_cl); # Start with at least the generators of ClK
    while (fb_all[k].norm().log().log()*n < 1/(un_rk+k)*(ln(sqrt(n+k))-ln(2)*r2/2.+k*ln(n)+ln(hR)+sum(log(n*log(pp.norm())) for pp in fb_all[:k])) - ln(sqrt(1+1/(n+k)))):
        k = k+1;
...
def twphs_get_dimfb(K, method, fb_cl, fb_oth):
    ...
    if ((method == 'PHS') or(method=='MYPHS')): return phs_get_dimfb(K);
    if ((method == 'OPT')  or  (method=='MYOPT')): return opt_get_dimfb(K, fb_cl, fb_oth);
    if (method == 'TW') or (method == 'MYTW') or (method=='MYOPT0') or (method=='MYPHS0'):  return tw_get_dimfb(K, fb_cl, fb_oth);
    #if (method== 'MYTW'):  return my_tw_get_dimfb(K, fb_cl, fb_oth);
...
def twphs_get_fb(K, method='TW', r=0, rand=False):
    ...
    if (method == 'PHS') or (method == 'MYPHS') or (method == 'NONE' and rand == True): # rand ignored if method != 'FORCED'
...
def my_phs_get_fb(K):
    return twphs_get_fb(K, method='MYPHS');

def my_phs0_get_fb(K):
    return twphs_get_fb(K, method='MYPHS0');

def opt_get_fb(K):
    return twphs_get_fb(K, method='OPT');

def my_opt_get_fb(K):
    return twphs_get_fb(K, method='MYOPT');

def my_opt0_get_fb(K):
    return twphs_get_fb(K, method='MYOPT0');

def tw_get_fb(K):
    return twphs_get_fb(K, method='TW');

def my_tw_get_fb(K):
    return twphs_get_fb(K, method='MYTW');
...
def mytws_get_twfHcE_matrix(r1, r2, fb, b_prec=fp.BIT_PREC_DEFAULT):
    ...
    # Always project on H1
    _span_H = (_n+_k);
    _pH  =my_get_projection_H(_n, _k,fb, b_prec=b_prec);
    #v_l=vector([RealField(b_prec)(1)]*_span_H);
    #for i in range(0,_n):
        #v_l[i]=(1/RealField(b_prec)(_n));
    
    #print("thisline",(v_l*_pH).base_ring().precision());
    #assert (fp.fp_check_zero("PrH(1)", (v_l*_pH).coefficients(), target=b_prec));
    
    # Scaling by c (done before because of the 'NONE' iso type
    _c    =RealField(b_prec)(1);
    _c_Id = block_diagonal_matrix(_c*identity_matrix(_n), identity_matrix(len(fb)));
    assert (_c_Id.base_ring().precision() >= b_prec);
    
    # Full-rank strategy
    
   # MK-isometry on _n+_k (whole space)
    _fH = my_get_minkH(r1, r2, _k, fb,b_prec=b_prec);
...
def myphs_get_twfHcE_matrix(r1, r2, fb, b_prec=fp.BIT_PREC_DEFAULT):
    ...
    _sum=sum(ln(RealField(b_prec)(_p.norm())) for _p in fb);
    _sum_k=RealField(b_prec)(_sum/len(fb));
    ...
    # Scaling by c (done before because of the 'NONE' iso type
    #_c      = twphs_get_c(_n, fb, 'MYPHS', b_prec=b_prec);
    _c =RealField(b_prec)(1);
    _c_Id   = block_diagonal_matrix(_c*identity_matrix(_n), 2*_sum_n/3*identity_matrix(len(fb)));
    assert (_c_Id.base_ring().precision() >= b_prec);
...
def myphs0_get_twfHcE_matrix(r1, r2, fb, b_prec=fp.BIT_PREC_DEFAULT):
    ...
    # Scaling by c (done before because of the 'NONE' iso type
    #_c      = twphs_get_c(_n, fb, 'OPT', b_prec=b_prec);
    _c =RealField(b_prec)(1);
    _c_Id   = block_diagonal_matrix(_c*identity_matrix(_n),RealField(b_prec)(4.5)*_sum_n*identity_matrix(len(fb)));
    assert (_c_Id.base_ring().precision() >= b_prec);
...
def myopt_get_twfHcE_matrix(r1, r2, fb, b_prec=fp.BIT_PREC_DEFAULT):
    ...
    #_c      = twphs_get_c(_n, fb, 'OPT', b_prec=b_prec);
    _c =RealField(b_prec)(1);
    _c_Id   = block_diagonal_matrix(_c*identity_matrix(_n), RealField(b_prec)(3.5)*_sum_n*identity_matrix(len(fb)));
    assert (_c_Id.base_ring().precision() >= b_prec);
...
def myopt0_get_twfHcE_matrix(r1, r2, fb, b_prec=fp.BIT_PREC_DEFAULT):
    ...
    #_c      = twphs_get_c(_n, fb, 'OPT', b_prec=b_prec);
    _c =RealField(b_prec)(1);
    _c_Id   = block_diagonal_matrix(_c*identity_matrix(_n), 4*_sum_n*identity_matrix(len(fb)));
    assert (_c_Id.base_ring().precision() >= b_prec);
...
def get_twfHcE_matrix(r1, r2, fb, method, b_prec=fp.BIT_PREC_DEFAULT):
    ...
    if(method=='MYTW'):
        return mytws_get_twfHcE_matrix(r1, r2, fb, b_prec=b_prec);
    if(method=='MYOPT'):
        return myopt_get_twfHcE_matrix(r1, r2, fb, b_prec=b_prec);
    if(method=='MYOPT0'):
        return myopt0_get_twfHcE_matrix(r1, r2, fb, b_prec=b_prec);
    if(method=='MYPHS'):
        return myphs_get_twfHcE_matrix(r1, r2, fb, b_prec=b_prec);
    if(method=='MYPHS0'):
        return myphs0_get_twfHcE_matrix(r1, r2, fb, b_prec=b_prec);
    ...
    if (method == 'NONE'): # Do nothing.
        ...
        _fH = block_diagonal_matrix([get_minkH(r1,r2,0,b_prec=b_prec), identity_matrix(_k)], subdivide=False);
        assert (fp.fp_check_zero("Vol(fH)==1", [vol(_fH.transpose())-RealField(b_prec)(1)],
                                 target=b_prec, sloppy=True)); # /!\ Sloppy
...
def twphs_get_matrix(un, s_un, p_inf, fb, method, b_prec=fp.BIT_PREC_DEFAULT):
    ...
    _fb_type  = 'TWISTED' if ((method == 'TW') or (method=='MYTW')) else 'FLAT'; # flat is just -vp(elt).
    ...
    #_k=len(fb);
    #if(method=='MYTW'):
       # for i in range(_n,_n+_k):
           # _Log_sun.rescale_col(i,1/RealField(b_prec)(_n));
...
def my_phs_get_matrix(un, s_un, p_inf, fb, b_prec=fp.BIT_PREC_DEFAULT):
    return twphs_get_matrix(un, s_un, p_inf, fb, b_prec=b_prec, method='MYPHS');
...
def my_tw_get_matrix(un, s_un, p_inf, fb, b_prec=fp.BIT_PREC_DEFAULT):
    return twphs_get_matrix(un, s_un, p_inf, fb, b_prec=b_prec, method='MYTW');


def my_opt_get_matrix(un, s_un, p_inf, fb, b_prec=fp.BIT_PREC_DEFAULT):
    return twphs_get_matrix(un, s_un, p_inf, fb, b_prec=b_prec, method='MYOPT');
...
def twphs_guess_beta(a, bkzlat, K, fb, method):
    max_fb=0;
    for i in range(len(fb)):
        if (ln(fb[i].norm())>max_fb):
            max_fb=ln(fb[i].norm());
    _sum = sum(bkzlat.base_ring()(ln(_pid.norm())) for _pid in fb);
    _sum_k=_sum/len(fb);
    _n = K.degree();
    _k = len(fb);
    _sum_n=_sum/((1+ln(_n))*_n);
    _sum_nn=_sum/_n**1.5;

    if((method=='PHS') or (method=='OPT')):
        return (vol_reduced(bkzlat)-bkzlat.base_ring()(0.8));

    if(method == 'MYPHS'):
        return  bkzlat.base_ring()(vol_reduced(bkzlat)/(_sum_nn)); 
    
    if(method == 'MYPHS0'):
        return bkzlat.base_ring()(vol_reduced(bkzlat)/(4*_sum_nn)); 

    if(method == 'MYOPT'):
        return bkzlat.base_ring()(vol_reduced(bkzlat)/(3*_sum_n)); 

    if (method == 'MYOPT0'):
        return bkzlat.base_ring()(vol_reduced(bkzlat)/(4*_sum_n)); 
    ...
    if(method=='MYTW'):
        _vol = vol_reduced(bkzlat)/(5);   
        #return bkzlat.base_ring()( (_k+_n)/_n*_vol + ln(a.norm())/_n);
        return bkzlat.base_ring()( (_k+_n)/_n*_vol + ln(a.norm())/_n - _sum/_n - ln(fb[0].norm()));
...
def twphs_get_target(eta, a, p_inf, fb, method, beta=0.0, b_prec=fp.BIT_PREC_DEFAULT, _pcmp_fhce=0):
    ...
    #if(method=='MYTW'):
       # for i in range(_n,_n+len(fb)):
           # _log_t[i]=(1/RealField(b_prec)(n))*RealField(b_prec)(_log_t[i]);
    assert (_log_t.base_ring().precision() >= _work_prec);
    _log_t.change_ring(RealField(b_prec));
    if (method == 'TW'): # Otherwise, there is nothing we could check
        assert (fp.fp_check_zero("sum(log_t)-N(a)", [sum(_log_t)-a.norm().log(prec=b_prec)], target=b_prec));
    #if (methon == 'MYTW'): # Otherwise, there is nothing we could check
       # assert (fp.fp_check_zero("sum(log_t)-N(a)", [sum(_log_t)-(1/RealField(b_prec)(n))*a.norm().log(prec=b_prec)], target=b_prec));
