...
def my_get_minkH(r1, r2, k,fb, b_prec=fp.BIT_PREC_DEFAULT):
    _dim   = r1+r2+k;
    _B_one = matrix([ vector([1 if (_i == _j) else (-1 if (_i == _j+1) else 0) for _j in range(_dim)])
                      for _i in range(1,_dim) ]);
    # To Minkowski space
    ...
    for i in range(r1+2*r2):
        _B_t.rescale_col(i,RealField(b_prec)(6));

    for i in range(r1+2*r2,r1+2*r2+k):
        _B_t.rescale_col(i,RealField(b_prec)(5));
    _mH    = matrix(RealField(b_prec), _B_t);
    _OmH,_ = gram_schmidt_ortho(_mH, normalize=True);
...
def my_get_projection_H(n, k,fb, b_prec=fp.BIT_PREC_DEFAULT): 
    _pH  = (-1/RealField(b_prec)(n+k))*ones_matrix(n+k) + identity_matrix(n+k);
    ...
    for i in range(n):
        _pH.rescale_col(i,RealField(b_prec)(6));

    for i in range(n,n+k):
        _pH.rescale_col(i,RealField(b_prec)(5));


    assert (_pH.base_ring().precision() >= b_prec);
    return _pH;
