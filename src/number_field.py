...
#len(l1)=len(l2)
#compute \prod l1[i]^l2[i]
#def power_product(l1,l2):
   # result=1;
    #for i in range(len(l1)):
      #  result=result*(l1[i]**l2[i]);
    #return result;

def get_id_clDL(K,I,fb=[],s_fb=[],scramble=false):
    
    assert(is_prime(I) and I not in fb);
    _r1,_r2=K.signature();
    _I_fact=[I];
    _FBI=fb+_I_fact;
    ...
    t=cputime();
    _SU=get_S_units(K,_FBI)[1];
    t=cputime(t);
    _SS=[_su for _su in _SU[0]];
    _SS+=[_su for _su in _SU[1]];
    _SS=_SS[_r1+_r2-1:];
    assert(len(_SS)==len(fb)+1);
    
    
    #counstruct CIDL linear systrm from the relation matrix
    _sys_clDL= Matrix([[s.valuation(_pid) for _pid in _I_fact] for s in _SS]);
    v=vector([I.valuation(_pid) for _pid in _I_fact]);  
                     
    #solution space
    _clDL=_sys_clDL.solve_left(v);
    _clDL_ker=kernel(_sys_clDL).basis();
       
    
    if(scramble==true):
        if(len(s_fb)==0):
            ss_fb=get_S_units(K,fb)[1];
            s_fb=[s for s in ss_fb];

        _clDL+=vector([ ZZ.random_element(-2,3) for _k in range(_clDL_ker.nrows())])*_clDL_ker;
        #s_fb_pow=[s_fb[i]^_pow for i in range(len(s_fb)) for _pow in ZZ.random_element(-2,3)];
        #s_fb_pow=[];
        #for i in range((len(s_fb))):
           # temp=s_fb[i]^(ZZ.random_element(-2,3));
           # s_fb_pow.append(temp);
        #_sun=prod(s_fb_pow[i] for i in range(s_fb_pow));
        rand_b=[];
        for i in range((len(s_fb))):
            rand_b.append(ZZ.random_element(-2,3));
        _sun=prod(map(pow,s_fb,rand_b));
    else:
        _sun=K(1);
                     
    #_eta=_sun*power_product(_SS,_clDL);
    _eta=_sun*prod(map(pow,_SS,_clDL));
    return _eta;
...
# K=Q[a],p_inf 可以理解为r1+r2个典范映射对应的的向量空间，len(p_inf)=r1+r2，_phi就是所对应的每个对应的向量的空间
